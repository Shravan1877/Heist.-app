...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number; resolution: THREE.Vector2 }>((props, ref) => {
  const { colorNum, pixelSize, resolution } = props;
  const WrappedRetroEffect = wrapEffect(RetroEffectImpl);
  // @ts-ignore - wrapEffect might not perfectly type custom properties
  return <WrappedRetroEffect ref={ref} colorNum={colorNum} pixelSize={pixelSize} resolution={resolution} />;
});
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    if (u.bgColor.value.r !== bgColor[0] || u.bgColor.value.g !== bgColor[1] || u.bgColor.value.b !== bgColor[2]) {
    ...
    <>
      ...
      <EffectComposer>
        <RetroEffect 
          colorNum={colorNum} 
          pixelSize={pixelSize} 
          resolution={waveUniformsRef.current.resolution.value} 
        />
      </EffectComposer>
